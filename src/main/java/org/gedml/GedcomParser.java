 ...
 * <p>The events sent to the ContentHandler reflect the GEDCOM input "as is"; there is no validation or conversion of tags.</p>
 *
...
public class GedcomParser implements XMLReader, Locator{
	...
	private static final List<String> ACCEPTED_TRUE_SAX_FEATURES = Arrays.asList("http://xml.org/sax/features/namespace-prefixes",
			...
			while((line = reader.readLine()) != null){
				...
				if(line.length() > 0){
					...
					if(!lineParser.parse(line)){
						if(goodLine){
							errorHandler.error(new SAXParseException("Line does not appear to be standard @ " + this.getLineNumber() + " appending content to the last tag started." + line, this));
