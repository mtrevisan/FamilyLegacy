...
public class GedcomTagLine extends GedcomLine{
	...
	public GedcomTagLine setValue(String value){
		if(! getStoreLine().hasValueNames()){
			throw new GedcomAccessError("Line " + getTag() + " in " + getStoreLine().getParentBlock().getStoreStructure().getStructureName() + " does not have an value-field.");
		}
		...
		if(possibleValues.size() > 0 && ! possibleValues.contains(value)){
			throw new GedcomAccessError(value + " is not a possible value for line " + getTag() + " in " + storeLine.getParentBlock().getStoreStructure().getStructureName() + ". Possible values are: " + GedcomFormatter.makeOrList(possibleValues, "", ""));
		}
	...
	public GedcomTagLine setXRef(String xref){
		if(! getStoreLine().hasXRefNames()){
			throw new GedcomAccessError("[ERROR] Line " + getTag() + " in " + getStoreLine().getParentBlock().getStoreStructure().getStructureName() + " does not have an xref-field.");
		}
