...
public class GedcomHelper{
	 ...
	 * @return
	 */
	public static int getAge(ZonedDateTime birthDate){
		return getAge(birthDate, ZonedDateTime.now());
	}
	 ...
	 * @return
	 */
	public static int getAge(ZonedDateTime fromDate, ZonedDateTime toDate){
		if(fromDate == null || toDate == null){
			return 0;
		}

		ZonedDateTime bd = fromDate.truncatedTo(ChronoUnit.DAYS);
		ZonedDateTime now = toDate.truncatedTo(ChronoUnit.DAYS);
	 ...
	 */
	public static int isBeforeOrAfter(ZonedDateTime date, ZonedDateTime isBeforeOrAfter){
		if(isBeforeOrAfter.isBefore(date))
			return BEFORE;
		else if(isBeforeOrAfter.isAfter(date))
			return AFTER;
		else
			return SAME;
	}
