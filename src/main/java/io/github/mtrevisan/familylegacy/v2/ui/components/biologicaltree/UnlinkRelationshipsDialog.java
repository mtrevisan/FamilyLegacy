...
public class UnlinkRelationshipsDialog extends JDialog{
	...
	private void extractRelationships(final List<RelationshipInfo> parents,
		final List<RelationshipInfo> partners,
		final List<RelationshipInfo> children){
		...
		for(final FLEFRecord relationship : relationships){
			...
			if(isChildRel){
				if(individualId.equals(subjectId))
					...
					parents.add(new RelationshipInfo(relId, otherId, otherName));
				else
					// individual is the parent -> other is a child
					children.add(new RelationshipInfo(relId, otherId, otherName));
			}
			else if(isPartnerRel)
				partners.add(new RelationshipInfo(relId, otherId, otherName));
