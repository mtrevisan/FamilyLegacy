 ...
 * Supports filtering by event type, date range, and geographic location.
 */
public class HistoricEventSearchStrategy implements SearchStrategy{
	...
	private String historicEventType;
	...
	public Predicate<FLEFRecord> buildPredicate(final SearchCriteria criteria, final FLEFModel model){
		historicEventType = criteria.getFilterFor("historicEventType");
		...
		return historicEvent -> {
			// Historic event type filter
			if(StringUtils.isNotEmpty(historicEventType)){
				final String recordType = FLEFRecordHelper.getChildValue(historicEvent, TAG_TYPE);
				if(!historicEventType.equalsIgnoreCase(recordType)){
					return false;
				}
			}
			...
			if(StringUtils.isNotEmpty(locationContains)){
				final String placeRef = FLEFRecordHelper.getChildValue(historicEvent, TAG_PLACE_PLACE);
				if(placeRef != null){
	...
	private boolean isDateInRange(final FLEFRecord dateRecord){
		...
		final int eventMinYear = yearRange[0];
		final int eventMaxYear = yearRange[1];
