...
public final class EventIndex{
	...
	public static EventIndex build(final FLEFModel model){
		...
		for(final FLEFRecord event : model.getRecordsByType(EventHandler.TYPE)){
			...
			final EventDatum datum = new EventDatum(id, type != null? type: "unknown",
				description != null? description: "", date, placeId, placeName, participants);
			events.add(datum);
			byId.put(id, datum);
		}
		...
		return new EventIndex(Collections.unmodifiableList(events),
			freeze(byType), freeze(byParticipant), freeze(byPlace),
	...
	private static Map<String, List<EventDatum>> freeze(final Map<String, List<EventDatum>> source){
		final Map<String, List<EventDatum>> result = new HashMap<>(source.size());
		for(final Map.Entry<String, List<EventDatum>> e : source.entrySet())
