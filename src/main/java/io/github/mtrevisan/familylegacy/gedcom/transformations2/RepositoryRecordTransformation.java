...
public class RepositoryRecordTransformation implements Transformation<Gedcom, Flef>{
	...
	private void contactStructureTo(final GedcomNode parent, final GedcomNode destinationNode){
		...
		for(final GedcomNode url : urls)
			contact.addChildValue("URL", url.getValue());
		destinationNode.addChild(contact);
	}
